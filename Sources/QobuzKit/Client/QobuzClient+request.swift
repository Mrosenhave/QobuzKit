...
public extension QobuzClient {
  ...
  func request<T: Endpoint>(_ endpoint: T, token: String? = nil) async throws -> T.Response {
    ...
    do {
      ...
      switch httpResponse.statusCode {
              ...
              case 400:
                  ...
                  throw NetworkError.badRequest(message: errorResponse?.message ?? "Bad request")
              case 401:
                  let errorResponse = try? JSONDecoder().decode(QBError.self, from: data)
                  throw NetworkError.unauthorized(message: errorResponse?.message ?? "Unauthorized")
              case 402:
                  let errorResponse = try? JSONDecoder().decode(QBError.self, from: data)
                  throw NetworkError.requestFailed(message: errorResponse?.message ?? "Request failed")
              case 404:
                  let errorResponse = try? JSONDecoder().decode(QBError.self, from: data)
                  throw NetworkError.notFound(message: errorResponse?.message ?? "Not found")
              case 500...599:
                  let errorResponse = try? JSONDecoder().decode(QBError.self, from: data)
                  throw NetworkError.serverError(message: errorResponse?.message ?? "Server error")
              default:
                  throw NetworkError.invalidResponse
              }
